...
class PostgresWriter(object):
    ...
    def write_constraints(self, table):
        ...
        for key in table.foreign_keys:
            ...
            REFERENCES "%(ref_table_name)s"(%(ref_column_name)s) %(cascade)s;""" % {
                'table_name': table.name,
                'column_name': ', '.join('%s' % col for col in key['column']),
                'ref_table_name': key['ref_table'],
                'ref_column_name': ', '.join('%s' % col for col in key['ref_column']),
                'cascade' : 'ON DELETE CASCADE' if key['cascade'] else ""})
